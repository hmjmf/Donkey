...
#include "dataStructure/skip_list.hpp"

TEST(skip_lists, skip_lists_node_base){
  Donkey::skip_lists_node<uint>* node = new Donkey::skip_lists_node<uint>(10, 12);
  for (int i = 0; i < 12 ; i++){ 
   Donkey::skip_lists_node<uint>* node_ = new Donkey::skip_lists_node<uint>(i, 4);
...
TEST(skip_lists, base){
  auto sl = Donkey::skip_list<int>::create();
  sl -> insert(1);
  sl -> insert(0);
  sl -> insert(-100);
  sl -> insert(100);
  ...
  auto sl_str = Donkey::skip_list<std::string>::create();
  sl_str -> insert("1");
  sl_str -> insert("0");
  sl_str -> insert("-100");
  sl_str -> insert("100");
...
TEST(skip_lists, iterator){
  ...
  int index = 0;
  for (Donkey::skip_list<int>::iterator i=sl->begin(); i!=sl->end(); ++i){
    //std::cout<< *i << std::endl;
    //EXPECT_EQ(*i ,v[index]);
    index++;
  }
...
TEST(skip_lists, class_obj){
  auto sl = Donkey::skip_list<t>::create();
...
void insert(const std::vector<int>& src, const std::shared_ptr<Donkey::skip_list<Key_t>> sl){
  for(auto i = src.begin(); i != src.end(); i++){
    sl->insert(*i);
  }
}
template <typename Key_t >
void contains(const std::vector<int>& src, std::shared_ptr<Donkey::skip_list<Key_t>> sl, std::promise<std::vector<bool>>& promiseObj){
  std::vector<bool> res;
  for(auto i = src.begin(); i != src.end(); i++){
...
TEST(skip_lists, multithreading){
  auto sl = Donkey::skip_list<int>::create();
  int thread_num = 8;
...
//contains not in
  ...
  auto sl_SG = Donkey::skip_list<int>::create();
